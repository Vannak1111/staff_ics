...
class LoginController extends GetxController {
  ...
  void login() {
    ...
    userLogin(emailController.value.text.trim(),
            passwordController.value.text.trim(), storage.read('device_token'))
        .then((value) {
      try {
  ...
  Future userLogin(String email, String password, String firebaseToken) async {
    ...
    try {
      var response =
          await _dio.post("${dotenv.get('baslurl_school')}api/login", data: {
        'email': '${email}',
        'password': '${password}',
